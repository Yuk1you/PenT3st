...
> **v2.1.0**（2026-09-14）：定位收敛为「授权渗透测试工作流」。众测/SRC 作业纪律从行为纪律节移出（归档于 `../PenT3st-removed-src-rules.md`）；Phase 1 新增「作业模式」判定（默认渗透测试，**不主动建议红队**）；持久化改为按作业模式条件分支；`compliance.md` 成为动作尺度唯一裁决点；`payloader/` 条目库接入路由；删除重复的 MCP 章节。v2.0.0 前版归档于 `../PenT3st-V1.0.0`。
...
| **作业模式（v2.1.0）** | Phase 1 判定「渗透测试（默认）/ 红队评估」。红队**需用户明确指定**，skill 不主动建议、不暗示。两种模式仅 3 处分歧：Phase 3 目标反推 + 达成即停、Phase 4 一律最高隐蔽、持久化按 RoE 条款 |
| **两层知识库** | `playbooks/` 是流程手册（探测→绕过→利用），`payloader/` 是条目库（条目带前置条件/语法解析/tags）。两层都接进路由，不凭记忆出 payload |
...
| **合规红线内嵌** | `compliance.md` 是**动作尺度（提权 / 横向 / 持久化能到哪一步）的唯一裁决点**，定义绝对禁止 / 需确认 / 必做 / 中止触发条件；持久化按作业模式条件分支（RoE 无明文条款即禁止）。Phase 5 报告前强制比对 |
...
└── references/                       # 厚知识库，按需 Read
    ...
    ├── payloader/                    # 条目库（53 文件）：原始富数据 payload，每条带前置条件/攻击链/语法解析/tags
    │   ├── index.md                  # 分类索引
    │   ├── waf-bypass.md             # WAF/EDR 绕过变体库
    │   ├── by-category/web|intranet/ # 按类目拆分的 payload 条目
    │   ├── tools/                    # 工具命令速查
    │   └── raw/                      # ⚠️ 1.7MB 源数据 JSON — skill 不读，勿加载
    ├── dictionaries/                 # 字典与指纹
    │   ├── 00-index.md
...
| 阶段 | 角色 | MUST 输出 | 关键约束 |
|------|------|----------|---------|
| **Phase 1 · Scope** | Planner | **作业模式** + 授权确认 + 目标范围 + 测试约束（四项缺一不进；红队模式额外两项：评估目标 / 持久化许可） | 无书面授权 = 违法，无例外；**模式默认渗透测试，不主动建议红队** |
