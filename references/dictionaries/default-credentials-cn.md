...
> 区别于国际通用字典（如 SecLists `default-passwords`），本表面向**国内渗透测试常见目标**：政企 OA、运营商网管、国产 CMS、国产监控、国产中间件。
...
> 银行后台默认凭据通常严格修改——以下为运维 / 测试环境常见，**仅在授权测试资产中使用**。
...
- **禁**：在政府 / 国企 / 银行 / 运营商系统使用本字典而无授权测试委托。这些行业属于关键基础设施。
