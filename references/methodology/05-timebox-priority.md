# 渗透测试时间盒优先级——基于 22,132 案例统计的高危占比排序

> 视角：渗透测试工程师在限定时间内（一次渗透测试项目、一次 HVV 演练、一次专项评估）应该按什么顺序投入时间。
> 配套读物：`01-attack-priority.md`（讲漏洞类型本身影响多大）+ 本篇（讲在一类漏洞中"挖到 == 高危"的概率）。

---

## 1. 一句话原则

`01-attack-priority.md` 给的是 **"如果挖到影响多大"**；
本篇给的是 **"挖一个 == 高危的概率有多大"**。
两者结合 → 时间投入回报率最高的先打。

```
时间盒回报 = 漏洞基线影响（01 表）  ×  挖到即高危概率（本表）  /  平均探测代价
```
...
> 22,132 个真实样本，"高危"=评定为 high 或 critical。
...
| 14（信息泄露） | `playbooks/info-disclosure.md` + `dictionaries/chinese-fingerprints.md` |
