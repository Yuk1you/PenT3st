...
银行/金融业的漏洞特点：**资金类漏洞占比小但单发价值高**，**鉴权层 + 接口层是主战场**。一个支付绕过的渗透影响可以等于普通 SaaS 一年的全部业务损失。
...
```
methodology/05-timebox-priority.md   →  金融行业的 88% / 83% 重点漏洞
...
dictionaries/chinese-fingerprints.md  →  金融常见组件指纹（致远 OA / 用友 / 金蝶等）
```
