...
详见 `dictionaries/chinese-fingerprints.md`。
...
> 短信网关、核心网、AAA、实名 — 这四个属于"敏感目标禁区"，即使授权渗透测试也不要深入。证明可访问即停。
...
```
methodology/05-timebox-priority.md   →  弱口令/越权/未授权 时间盒
...
dictionaries/chinese-fingerprints.md  →  致远 / 通达 / 万户 / SP 平台路径
```
