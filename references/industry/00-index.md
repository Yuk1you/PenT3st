...
| 行业 | 常见评估入口 |
|------|--------------|
| 银行 / 金融 | 金融机构委托渗透测试、HVV 金融行业专项、安全评估授权项目 |
| 电信 / ISP | 运营商委托安全评估、HVV 运营商行业专项 |

**红线**：金融、电信都属于关键基础设施，证据纪律比通用安全评估更严格——不写、不删、不改、不留 webshell。详见 `methodology/03-evidence-discipline.md`。
...
```
methodology/05-timebox-priority.md   →  按时间盒选行业内的优先漏洞
playbooks/<具体类型>.md                  →  落实漏洞类型的探测/利用
dictionaries/default-credentials-cn.md   →  行业内 CN 厂商默认凭据
dictionaries/chinese-fingerprints.md  →  行业组件指纹与默认路径
```
