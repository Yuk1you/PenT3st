...
1. **不准凭记忆出 payload**。要给 SQLi/RCE/SSRF/XSS 任何 payload 前，先 Read 对应 `references/playbooks/<type>.md`（或 `<type>/00-index.md` + 具体子文件，见下表）。payload 必须能在文件里查到出处。
   ...
   **两层知识库的分工**（都要用，别只用一层）：
   ...
   | 层 | 路径 | 性质 | 何时读 |
   |---|---|---|---|
   | **流程层** | `references/playbooks/` | 作业手册：探测 → 绕过 → 利用 → 证据要点 | 默认入口，按 Phase 3 的 Playbook 路由表读 |
   | **条目层** | `references/payloader/` | 原始富数据：每条 payload 带**前置条件 / 攻击链 / 语法解析 / tags / platform** | playbook 里的 payload 不够用、要看前置条件或参数含义时；被 WAF 拦时先读 `methodology/02-bypass-toolkit.md` 决策树，不够再读 `payloader/waf-bypass.md` |

   ⚠️ **payloader 只读 `by-category/*.md` 与 `tools/*.md`**。`payloader/raw/` 是 1.7MB 的源数据 JSON（`web.json` 单文件 1.1MB），**不要读**。
...
**A. 情报与覆盖**
1. **测绘数据有时效**：外部测绘/EASM 快照（子域、存活、指纹）可能过期，测前先自行验活（如 DNS/HTTP 探测），别信过期的资产清单。
2. **负结果诚实记录**：负结论（排除/闭合/不可达）也要落盘成档，如实登记——收尾时据此产出**可核的工作覆盖声明**，也避免后续重复打已闭合资产。

**B. 会话纪律**
3. **会话内清单化机制**：用户口头规则立即写入会话持续检查清单，每次探测前强制过一遍，避免"用户说完跟没听见一样"。

**C. 技术判别纪律**
4. **证据自动化落盘**：探测脚本自动落盘 `summary.txt` + 关键响应**原始包**（+脚本+日志三件套）；只留原始数据，不加自造字段。
5. **响应异常三分法**：连接/响应异常先三分判别——本地准入劫持 / 目标 WAF / 服务端限流（用响应体特征指纹），再对症，不误判目标防护。
6. **准确接口名先提取**：不盲猜接口方法名，先 grep 已固化页面提取准确 `.do`/端点名再发请求（盲猜浪费请求量且易触发服务端限流）。
7. **横向扩展 + 边界确认**：发现单点越权/IDOR 后用字典/相邻号扩展，并做 3 跨度 + 边界外对照确认数据边界，量化影响面。

**扩大利用原则**：有漏洞就继续利用扩大战果，不要停在"发现"。发现 → 验证 → 扩大利用（拉数据/提权/横向）→ 停在拿不到更多为止，或停在授权/RoE 边界（边界见 `references/compliance.md`）。
...
| **前端逆向 / 反混淆 / 动态调试** | **jshookmcp**（本机已装） | 手动用 devtools 分析，或读 `playbooks/xss/00-index.md` 的静态绕过手法 |
| **移动端 hook / SSL pinning** | **jshookmcp**（frida / adb 域） | 指导用户手动跑 frida 脚本 |
| **协议逆向 / WASM / Electron** | **jshookmcp**（wasm / platform / mojo-ipc 域） | 降级为黑盒探测 |

> **jshookmcp 详细工具地图**：`references/tools/mcp-jshook.md`——36 域索引 + 场景→工具映射 + profile 选择（默认 `search`，~3K token）+ Burp bridge 的替代路径。该文档里的工具名以实际 `search_tools` 返回值为准，**不臆造**。
...
| 模式 | 判定条件 |
|---|---|
| **渗透测试**（默认） | **除下行红队触发语之外的任何情况**——包括只说"渗透测试""测一下""打一下""看看能不能突破""内网渗透" |
| **红队评估** | 用户**明确说出**：红队评估 / 红队测试 / 红队演练 / 攻防演练 / HVV 红队 |

**硬性规则（不可逾越）**：

1. **拿不准 → 一律按渗透测试走**，不得自行升级为红队。
2. **本 skill 不得主动建议、暗示或询问"要不要切红队模式"**。用户没明说 = 渗透测试。即使发现目标含域控/核心资产、即使素材里躺着红队 payload，也不构成切换理由。
3. 已确认红队模式后，若用户后续表述回到普通渗透语境，**主动降回渗透测试**并说明原因。
4. 模式判定结果必须写进下面的 checkpoint，不写 = 未判定。

### MUST 输出 checkpoint（四项缺一不进 Phase 2，缺什么向用户问什么，不要假设）

- [ ] **作业模式**：渗透测试（默认）/ 红队评估（**需用户明确指定**，见上表）
- [ ] **授权确认**：已获得书面授权（渗透测试合同/授权书/邮件确认）—— 向用户确认
- [ ] **目标范围**：可测目标清单（域名 / IP / 网段 / 应用 / API）+ 禁测项（逐条列）
- [ ] **测试约束**：测试类型（黑盒/灰盒/白盒）、时间窗口、是否允许社工/DoS/数据破坏
  - **红队模式额外两项**（缺一不进 Phase 2）：
    - **评估目标**：要拿到什么 / 证明哪条路径可行 / 测什么检测能力
    - **持久化许可**：RoE 是否**明文**授予；未明文授予 = 按禁止处理
...
### 作业模式差异（Phase 3 是唯一的流程分歧点）

**渗透测试模式（默认）**：按下方 Playbook 路由表逐目标推进，追求**覆盖面**——时间盒内能测的 P0/P1 都测。

**红队评估模式**：从 Phase 1 记录的**评估目标反向链推**，只走链——

1. **反推缺口**：目标 → 达成它还缺什么（例：拿域控 → 需域管凭据 → 需某台入域主机的本地管理员 → …），逐层列出前置条件
2. **只打链上点**：**不在链上的目标即使 P0 也不投入**——红队不比覆盖率
3. **剪枝换链**：同一路径连试数种打法不通 / 前置条件不成立 / 目标版本已修 → **立即放弃换链**，不恋战不硬啃
4. **终止条件改为"目标达成即停"**：不看时间盒剩余量，不追求"再多找一个洞"

> 两种模式下 payload 都必须来自 `references/playbooks/` 或 `references/payloader/` 的**实际文件**，不准凭记忆生成。
...
8. **存量漏洞复测（多轮 / 长期项目）**：对上一轮报告中的漏洞做周期复测——已修复的保留原始证据（截图 + 原始响应）备查；未修复的可作为复测结论与催办依据；同域修复一个点后快速扫同 pattern 是否全量修复。同服务多协议暴露面可绕单端口封禁（续测前确认不违反授权范围）。
...
- 不破坏业务数据和服务可用性
- **持久化**：渗透模式下**禁止**（测试完成后清理所有痕迹）；红队模式下按 `references/compliance.md`「🔀 持久化与痕迹」的条件分支执行——**RoE 无明文条款即禁止**，本文件不重复定义
- 不访问与授权范围无关的敏感数据
- 发现关键漏洞（如域控/核心数据库）时先暂停，与甲方确认后再继续

### 隐蔽性要求（按作业模式）

- **渗透测试模式（默认）**：**不要求隐蔽**。真实约束是**不影响业务**——控速、避高峰，见 `references/compliance.md`「自动化大规模扫描」。
- **红队评估模式**：**一律按最高隐蔽要求**（不做分级）。MUST Read `references/playbooks/intranet-postexp/13-evasion.md` + `references/payloader/by-category/intranet/免杀与规避.md`；全程控制流量特征、禁止高频扫描、准备 **OPSEC 失败预案**（暴露后的处置与告知路径）。
